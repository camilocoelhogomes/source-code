...
| Versão | `0.1.1` |
| Design / BDD / Interfaces | `0.1.0` / `0.1.1` / `0.1.0` |
...
- Sem `docker build` / compose real; manifesto = asserts de arquivo.
- Sem domínio novo: sync/reconcile só via portas injetadas (I-T19-014/018).
- BDD (`tests/bdd/test_container_delivery.py` CD-01..10) permanece suíte de superfície; unitários cobrem extremos e contratos isolados.
- Pré-implementação: `ImportError` / `AssertionError` (artefatos/módulos ausentes) — **RED**.
...
| UT-M03 | Compose: serviços app/postgres/qdrant/zoekt/slm + ports UI/MCP | manifesto | M-T19-002; CD-08 |
| UT-M04 | Compose: volumes `/repos` + `CONFIG_PATH`; healthcheck `/healthz` | manifesto | M-T19-003; CD-08/09 |
| UT-M05 | `pyproject.toml` declara DEC-015 + `uvicorn` + grammars | deps | M-T19-006; CD-05 |
| UT-M06 | `.env.example` nomes canônicos (incl. UI/MCP) sem segredos reais | manifesto | M-T19-003; I-T19-009; CD-09 |
...
|---|---|---|---|
| Unit plan + suíte | `APPROVED_BY_ARCHITECT` | Tech Lead Architect | 2026-07-18 |
