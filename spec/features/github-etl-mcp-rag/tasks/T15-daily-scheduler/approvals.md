...
| Refatoração Blue (`refactoring.md`, `scheduler.py`: `_cron_trigger`/`_add_cron_job`) | — | `BLUE_APPROVED_BY_ARCHITECT` | Tech Lead Architect | 2026-07-18 |
...
Blue (Revisão 6, `reviews.md`): extração de `_cron_trigger`/`_add_cron_job` em
`scheduler.py` e simplificação de `active_cron()`, sem mudança de comportamento
ou contrato. Suíte completa reexecutada nesta revisão: **813 passed, 2 skipped**,
cobertura **98.85%**; `schedule/scheduler.py` **100%** linhas+branches, idêntico
ao baseline pré-Blue. **`BLUE_APPROVED_BY_ARCHITECT`**. Documentação
(`README.md`/`CHANGELOG.md`) já atualizada nesta task. Gate humano único
permanece no merge do PR.
